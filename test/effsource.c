...
int main(int argc, char* argv[])
{
  ...
  /* The coordinate struct is ordered like {r,theta,phi,t}*/
  struct coordinate x = {0, r_p, M_PI_2, 0.0};

  /* Disable the GSL error handler so that it doesn't abort due to roundoff errors */
  gsl_set_error_handler_off();

  /* Output the singular field for the m=2 mode in the r-theta plane and output it to eff_source_m2.txt*/
  FILE *save_file;

  save_file = fopen("phi_pi_0.txt", "w");
  ...
  for(double r=8.9; r<=11.1; r+=0.1)
  {
    ...
    for(double theta=M_PI_2-0.1; theta<=M_PI_2+0.1; theta+=0.011)
    {
      ...
      fprintf(save_file, "%.15g\t%.15g\t%.15g\t%.15g\t%.15g\t%.15g\t%.15g\n",
